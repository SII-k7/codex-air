# Codex AIR v0.4.0 Implementation Report

Date: 2026-08-05

Repository: https://github.com/yehyakin/codex-codex-air

## 1. Final architecture

Codex AIR keeps one controller and two bounded execution tiers:
...
```text
.agents/skills/
  codex-air/                 canonical full Skill
...
The old `$sol-luna` entry is explicit-only, redirects to `$codex-air`, and is scheduled for
removal in v0.5.0. It does not contain or start a second orchestration implementation.

## 8. Installation and backup result

The v0.4 installer uses state version 3 under `~/.codex/codex-air`. It installs the canonical
...
`/var/folders/4_/6qbh_56x08q9kk1thgr6b5yr0000gn/T/codex-air-migration.KpbhoT`

The stable source was installed globally at `~/.agents/skills/codex-air`, with the v0.4.x alias
at `~/.agents/skills/sol-luna` and state version 3 at `~/.codex/codex-air`. The final installer
backup is `~/.codex/codex-air/backups/20260804T223305Z-24556`. Installed files match the source,
`install.sh --check` returns success, and the complete `config.toml` hash is unchanged from the
pre-migration value.

## 9. Forward tests

The fixture suite covers Direct, Sol-only, explicit Codex AIR routing, independent parallel
...
PASS. A fresh `codex exec` session discovered `$codex-air`, loaded Chinese as the default language,
...
https://github.com/yehyakin/codex-codex-air. The repository rename is complete. The rename,
lifecycle, and identity-handshake implementation is
[`848b210`](https://github.com/yehyakin/codex-codex-air/commit/848b210691fcfd91ec8b5374ba7b35c19c48e18e).
Hosted evidence is [POSIX PASS](https://github.com/yehyakin/codex-codex-air/actions/runs/30956811267)
and [Windows PASS](https://github.com/yehyakin/codex-codex-air/actions/runs/30956811107). The
