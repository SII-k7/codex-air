> **Archived historical record.** This report describes an earlier release. Its model assignments, routing rules, commands, prices, paths, and status are not the current Codex AIR contract. Use the [documentation index](../../README.md) for current guidance.

# Codex AIR v0.4.0 Implementation Report

Date: 2026-08-05

Repository: historical predecessor of the current canonical Codex AIR repository
...
An external pre-change backup was created in a temporary directory before repository edits.

The stable source was installed globally at `~/.agents/skills/codex-air`, with the v0.4.x alias
at `~/.agents/skills/sol-luna` and state version 3 at `~/.codex/codex-air`. A persistent installer
backup was retained under the managed backup directory. Installed files match the source,
...
- the controller identity probe reported `gpt-5.6-sol`, effort `high`, read-only;
- its nested worker identity probe reported `gpt-5.6-luna`, effort `max`, read-only;
...
The historical release target was `v0.4.0` on `main`. The repository rename, lifecycle,
and identity-handshake implementation was bound to commit `848b210`. Hosted POSIX and Windows
checks passed before the annotated `v0.4.0` tag was created.
