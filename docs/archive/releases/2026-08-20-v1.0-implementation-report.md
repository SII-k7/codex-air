...
Repository: current canonical Codex AIR repository
...
`config.toml`, and global `AGENTS.md` to an isolated temporary directory.
...
v0.5 install to v1.0. The installed paths were
`~/.agents/skills/codex-air`, `~/.agents/skills/codex-prove`, and the three
`~/.codex/agents/air-*.toml` files. A persistent recovery backup was retained
under the managed backup directory.
...
After global v1 installation, a fresh ephemeral `codex exec` session loaded
`$codex-air` and completed a real Compatibility route. The Host launched every role with
...
- one fresh session discovered canonical `$codex-air`, selected all three exact
  `air-*` roles, and completed the
  Compatibility path plus Controller final review;
- a separate pre-AIR session was retained as compatibility evidence; current
  static and lifecycle tests cover the
