...
global_instructions="$base_dir/.codex/AGENTS.md"
if path_exists "$global_instructions"; then
  assert_plain "$global_instructions" file
  instructions=$(<"$global_instructions")
  if [[ "$instructions" == *'<!-- codex-air-default:start -->'* || "$instructions" == *'<!-- codex-air-default:end -->'* ||
        "$instructions" == *'<!-- codex-prove-default:start -->'* || "$instructions" == *'<!-- codex-prove-default:end -->'* ||
        "$instructions" == *'<!-- sol-control-default:start -->'* || "$instructions" == *'<!-- sol-control-default:end -->'* ]]; then
    die 'default routing is enabled; run bash scripts/default.sh disable before uninstalling'
  fi
fi
...
state_root="$base_dir/.codex/codex-air"
state_file="$state_root/install-state"
backup_root="$state_root/backups"
path_exists "$state_file" || die 'Codex AIR install state is missing'
assert_plain "$state_file" file
[[ "$(state_value "$state_file" version)" == 7 ]] || die 'unsupported Codex AIR install state'
...
managed_indexes=(0 1 5 6 7 15 16)
managed_keys=(skill_sha256 compat_skill_sha256 controller_sha256 complex_worker_sha256 efficient_worker_sha256 critical_controller_sha256 challenger_sha256)
for (( item=0; item<${#managed_indexes[@]}; item++ )); do
  ...
  path_exists "$target" || die 'an installed Codex AIR target is missing'
  assert_plain "$target" "${known_kinds[index]}"
  [[ "$(hash_path "$target" "${known_kinds[index]}")" == "$expected" ]] || die 'an installed Codex AIR target was modified; refusing to remove it'
done
...
if (( restore_latest )); then
  ...
  [[ "$(state_value "$manifest" version)" == 7 ]] || die 'latest backup manifest has an unsupported version'
...
for index in "${managed_indexes[@]}" 19; do
  target="$base_dir/${known_relatives[index]}"
