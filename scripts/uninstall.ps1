...
$managedIndexes = @(0, 1, 5, 6, 7, 15, 16)
$managedKeys = @("skill_sha256", "compat_skill_sha256", "controller_sha256", "complex_worker_sha256", "efficient_worker_sha256", "critical_controller_sha256", "challenger_sha256")
...
if ((Get-StateValue $state "version") -ne "7") { throw "unsupported Codex AIR install state" }
...
if ($RestoreLatest) {
    ...
    if ((Get-StateValue $manifest "version") -ne "7") { throw "unsupported backup manifest" }
...
try {
    foreach ($index in @($managedIndexes + 19)) {
        $target = Join-Path $baseDir $relativePaths[$index]
...
Write-Output "Uninstalled current Codex AIR version"
