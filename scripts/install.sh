...
canonical_skill_source="$REPO_ROOT/.agents/skills/codex-air"
compat_skill_source="$REPO_ROOT/.agents/skills/codex-prove"
controller_source="$REPO_ROOT/.codex/agents/air-controller.toml"
critical_controller_source="$REPO_ROOT/.codex/agents/air-critical-controller.toml"
...
for source_file in "$controller_source" "$critical_controller_source" "$complex_source" "$efficient_source" "$challenger_source"; do
...
if (( check_mode )); then
  ...
  validation_log=$(mktemp "${TMPDIR:-/tmp}/codex-air-validation.XXXXXX") || die 'cannot create a validation log'
...
if (( active_state_index >= 0 )); then
  ...
  case "$active_state_index:$state_version" in
    19:7)
      ...
      verify_owned 5 "$(state_value "$active_state" controller_sha256)"
      verify_owned 6 "$(state_value "$active_state" complex_worker_sha256)"
      verify_owned 7 "$(state_value "$active_state" efficient_worker_sha256)"
      verify_owned 15 "$(state_value "$active_state" critical_controller_sha256)"
      verify_owned 16 "$(state_value "$active_state" challenger_sha256)"
      owned[19]=1
      ;;
    20:6)
      verify_owned 1 "$(state_value "$active_state" skill_sha256)"
      verify_owned 2 "$(state_value "$active_state" compat_skill_sha256)"
      verify_owned 8 "$(state_value "$active_state" controller_sha256)"
      verify_owned 9 "$(state_value "$active_state" complex_worker_sha256)"
      verify_owned 10 "$(state_value "$active_state" efficient_worker_sha256)"
      verify_owned 17 "$(state_value "$active_state" critical_controller_sha256)"
      verify_owned 18 "$(state_value "$active_state" challenger_sha256)"
      owned[20]=1
      ;;
    20:5)
      verify_owned 1 "$(state_value "$active_state" skill_sha256)"
    ...
    21:3|21:4)
      ...
      verify_owned 11 "$(state_value "$active_state" sol_sha256)"
      verify_owned 13 "$(state_value "$active_state" luna_sha256)"
      terra_hash=$(state_optional "$active_state" terra_sha256)
      if [[ -n "$terra_hash" ]]; then verify_owned 12 "$terra_hash"; fi
      if [[ "$state_version" == 3 ]]; then
        verify_owned 3 "$(state_value "$active_state" compat_skill_sha256)"
      fi
      owned[21]=1
      ;;
    22:2)
      ...
      ;;
    23:1)
      verify_owned 4 "$(state_value "$active_state" skill_sha256)"
      verify_owned 14 "$(state_value "$active_state" sol_sha256)"
      verify_owned 13 "$(state_value "$active_state" luna_sha256)"
      owned[23]=1
      ;;
...
if (( check_mode )); then
  ...
  printf 'Install path: %s\n' "$base_dir/.agents/skills/codex-air"
  exit 0
fi
...
state_root="$codex_dir/codex-air"
...
copy_exact "$canonical_skill_source" "$transaction_dir/stage/codex-air"
copy_exact "$compat_skill_source" "$transaction_dir/stage/codex-prove"
copy_exact "$controller_source" "$transaction_dir/stage/air-controller.toml"
copy_exact "$critical_controller_source" "$transaction_dir/stage/air-critical-controller.toml"
copy_exact "$complex_source" "$transaction_dir/stage/air-complex-worker.toml"
copy_exact "$efficient_source" "$transaction_dir/stage/air-efficient-worker.toml"
copy_exact "$challenger_source" "$transaction_dir/stage/air-challenger.toml"
...
mv "$transaction_dir/stage/codex-air" "$base_dir/.agents/skills/codex-air"
mv "$transaction_dir/stage/codex-prove" "$base_dir/.agents/skills/codex-prove"
mv "$transaction_dir/stage/air-controller.toml" "$base_dir/.codex/agents/air-controller.toml"
mv "$transaction_dir/stage/air-critical-controller.toml" "$base_dir/.codex/agents/air-critical-controller.toml"
mv "$transaction_dir/stage/air-complex-worker.toml" "$base_dir/.codex/agents/air-complex-worker.toml"
mv "$transaction_dir/stage/air-efficient-worker.toml" "$base_dir/.codex/agents/air-efficient-worker.toml"
mv "$transaction_dir/stage/air-challenger.toml" "$base_dir/.codex/agents/air-challenger.toml"

if [[ "${ORCHESTRATE_FAILPOINT:-}" == after-replace ]]; then
  die 'injected failure after replacement'
fi

skill_hash=$(sha256_tree "$base_dir/.agents/skills/codex-air")
compat_hash=$(sha256_tree "$base_dir/.agents/skills/codex-prove")
controller_hash=$(sha256_file "$base_dir/.codex/agents/air-controller.toml")
critical_controller_hash=$(sha256_file "$base_dir/.codex/agents/air-critical-controller.toml")
complex_hash=$(sha256_file "$base_dir/.codex/agents/air-complex-worker.toml")
efficient_hash=$(sha256_file "$base_dir/.codex/agents/air-efficient-worker.toml")
challenger_hash=$(sha256_file "$base_dir/.codex/agents/air-challenger.toml")
for checksum in "$skill_hash" "$compat_hash" "$controller_hash" "$critical_controller_hash" "$complex_hash" "$efficient_hash" "$challenger_hash"; do
...
state_tmp=$(mktemp "$state_root/.install-state.XXXXXX") || die 'cannot create install state'
{
  ...
  printf 'critical_controller_sha256=%s\n' "$critical_controller_hash"
  printf 'complex_worker_sha256=%s\n' "$complex_hash"
  printf 'efficient_worker_sha256=%s\n' "$efficient_hash"
  printf 'challenger_sha256=%s\n' "$challenger_hash"
} >"$state_tmp"
...
printf 'Agent path: %s\n' "$base_dir/.codex/agents/air-controller.toml"
printf 'Agent path: %s\n' "$base_dir/.codex/agents/air-critical-controller.toml"
printf 'Agent path: %s\n' "$base_dir/.codex/agents/air-complex-worker.toml"
printf 'Agent path: %s\n' "$base_dir/.codex/agents/air-efficient-worker.toml"
printf 'Agent path: %s\n' "$base_dir/.codex/agents/air-challenger.toml"
