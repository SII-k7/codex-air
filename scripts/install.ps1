...
$canonicalSkillSource = Join-Path $repoRoot ".agents/skills/codex-air"
$compatSkillSource = Join-Path $repoRoot ".agents/skills/codex-prove"
$controllerSource = Join-Path $repoRoot ".codex/agents/air-controller.toml"
$criticalControllerSource = Join-Path $repoRoot ".codex/agents/air-critical-controller.toml"
...
foreach ($parent in @(
    ...
    (Join-Path $baseDir ".codex/codex-air")
)) {
...
if ($activeStateIndex -ge 0) {
    ...
    if ($activeStateIndex -eq 19 -and $version -eq "7") {
        Set-OwnedTarget 0 (Get-StateValue $state "skill_sha256")
        Set-OwnedTarget 1 (Get-StateValue $state "compat_skill_sha256")
        Set-OwnedTarget 5 (Get-StateValue $state "controller_sha256")
        Set-OwnedTarget 6 (Get-StateValue $state "complex_worker_sha256")
        Set-OwnedTarget 7 (Get-StateValue $state "efficient_worker_sha256")
        Set-OwnedTarget 15 (Get-StateValue $state "critical_controller_sha256")
        Set-OwnedTarget 16 (Get-StateValue $state "challenger_sha256")
        $owned[19] = $true
    }
    elseif ($activeStateIndex -eq 20 -and $version -eq "6") {
        Set-OwnedTarget 1 (Get-StateValue $state "skill_sha256")
        Set-OwnedTarget 2 (Get-StateValue $state "compat_skill_sha256")
        Set-OwnedTarget 8 (Get-StateValue $state "controller_sha256")
        Set-OwnedTarget 9 (Get-StateValue $state "complex_worker_sha256")
        Set-OwnedTarget 10 (Get-StateValue $state "efficient_worker_sha256")
        Set-OwnedTarget 17 (Get-StateValue $state "critical_controller_sha256")
        Set-OwnedTarget 18 (Get-StateValue $state "challenger_sha256")
        $owned[20] = $true
    }
    elseif ($activeStateIndex -eq 20 -and $version -eq "5") {
        Set-OwnedTarget 1 (Get-StateValue $state "skill_sha256")
        Set-OwnedTarget 2 (Get-StateValue $state "compat_skill_sha256")
        Set-OwnedTarget 8 (Get-StateValue $state "controller_sha256")
        Set-OwnedTarget 9 (Get-StateValue $state "complex_worker_sha256")
    ...
    elseif ($activeStateIndex -eq 21 -and @("3", "4") -contains $version) {
        Set-OwnedTarget 2 (Get-StateValue $state "skill_sha256")
        Set-OwnedTarget 11 (Get-StateValue $state "sol_sha256")
        Set-OwnedTarget 13 (Get-StateValue $state "luna_sha256")
        $terraHash = Get-OptionalStateValue $state "terra_sha256"
        if (-not [string]::IsNullOrEmpty($terraHash)) { Set-OwnedTarget 12 $terraHash }
        if ($version -eq "3") { Set-OwnedTarget 3 (Get-StateValue $state "compat_skill_sha256") }
        $owned[21] = $true
    }
    elseif ($activeStateIndex -eq 22 -and $version -eq "2") {
    ...
    elseif ($activeStateIndex -eq 23 -and $version -eq "1") {
        Set-OwnedTarget 4 (Get-StateValue $state "skill_sha256")
        Set-OwnedTarget 14 (Get-StateValue $state "sol_sha256")
        Set-OwnedTarget 13 (Get-StateValue $state "luna_sha256")
        $owned[23] = $true
    }
    else {
...
if ($Check) {
    ...
    Write-Output "Install path: $(Join-Path $baseDir '.agents/skills/codex-air')"
    exit 0
}

$skillsDir = Join-Path $baseDir ".agents/skills"
$agentsDir = Join-Path $baseDir ".codex/agents"
$stateRoot = Join-Path $baseDir ".codex/codex-air"
...
Copy-Exact $canonicalSkillSource (Join-Path $stageDir "codex-air")
Copy-Exact $compatSkillSource (Join-Path $stageDir "codex-prove")
Copy-Exact $controllerSource (Join-Path $stageDir "air-controller.toml")
Copy-Exact $criticalControllerSource (Join-Path $stageDir "air-critical-controller.toml")
Copy-Exact $complexSource (Join-Path $stageDir "air-complex-worker.toml")
Copy-Exact $efficientSource (Join-Path $stageDir "air-efficient-worker.toml")
Copy-Exact $challengerSource (Join-Path $stageDir "air-challenger.toml")
...
try {
    ...
    Move-Item -LiteralPath (Join-Path $stageDir "codex-air") -Destination (Join-Path $baseDir $relativePaths[0])
    Move-Item -LiteralPath (Join-Path $stageDir "codex-prove") -Destination (Join-Path $baseDir $relativePaths[1])
    Move-Item -LiteralPath (Join-Path $stageDir "air-controller.toml") -Destination (Join-Path $baseDir $relativePaths[5])
    Move-Item -LiteralPath (Join-Path $stageDir "air-critical-controller.toml") -Destination (Join-Path $baseDir $relativePaths[15])
    Move-Item -LiteralPath (Join-Path $stageDir "air-complex-worker.toml") -Destination (Join-Path $baseDir $relativePaths[6])
    Move-Item -LiteralPath (Join-Path $stageDir "air-efficient-worker.toml") -Destination (Join-Path $baseDir $relativePaths[7])
    Move-Item -LiteralPath (Join-Path $stageDir "air-challenger.toml") -Destination (Join-Path $baseDir $relativePaths[16])
    ...
    $controllerHash = Get-FileDigest (Join-Path $baseDir $relativePaths[5])
    $criticalControllerHash = Get-FileDigest (Join-Path $baseDir $relativePaths[15])
    $complexHash = Get-FileDigest (Join-Path $baseDir $relativePaths[6])
    $efficientHash = Get-FileDigest (Join-Path $baseDir $relativePaths[7])
    $challengerHash = Get-FileDigest (Join-Path $baseDir $relativePaths[16])
    foreach ($digest in @($skillHash, $compatHash, $controllerHash, $criticalControllerHash, $complexHash, $efficientHash, $challengerHash)) { Assert-Hash $digest }
...
Write-Output "Agent path: $(Join-Path $baseDir $relativePaths[7])"
Write-Output "Agent path: $(Join-Path $baseDir $relativePaths[16])"
