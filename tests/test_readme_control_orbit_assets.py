#!/usr/bin/env python3
"""Contracts for the bilingual Codex AIR README SVG assets."""
...
def worker_path_errors(root: ET.Element) -> list[str]:
    ...
    for element in tasks:
        ...
        if source != "air-controller":
            errors.append(f"task path must start at air-controller: {source!r}")
    ...
    for element in evidence:
        ...
        if target != "air-controller":
            errors.append(f"evidence path must return to air-controller: {target!r}")
        ...
        for name, path in ASSETS.items():
            with self.subTest(asset=name):
                ...
                if name.endswith("-zh.svg"):
                    ...
                    self.assertRegex(
                        text,
                        r"(?i)(?:target|budget|projection|sample[- ]validated)",
                        name,
                    )
                else:
                    self.assertIn("AIR", text, name)
                ...
                if name.startswith("hero-"):
                    self.assertIn("55%", text, name)
                    self.assertIn("Direct", text, name)
                    self.assertIn("Luna Primary", text, name)
                    self.assertIn("FILES", text, name)
                    self.assertIn("DIFF", text, name)
                    self.assertIn("TEST", text, name)
                    if name.endswith("-zh.svg"):
                        self.assertRegex(text, r"(?:目标|预算|比例|路由开销)", name)
                else:
                ...
                self.assertEqual(
                    ids.count("air-controller"),
                    1,
                    f"{name}: expected exactly one air-controller",
                )
                worker_ids = sorted(
                    identifier
    ...
    def test_worker_path_helper_requires_every_round_trip(self) -> None:
        def fixture(task_workers: tuple[str, ...], evidence_workers: tuple[str, ...]) -> ET.Element:
            ...
            for index, worker in enumerate(task_workers):
                ...
                path.set("data-from", "air-controller")
            ...
            for index, worker in enumerate(evidence_workers):
                ...
                path.set("data-to", "air-controller")
            return root
