#!/usr/bin/env python3
"""Contract tests for the bilingual Codex AIR documentation surface."""
...
CANONICAL_REPOSITORY_URL = "https://github.com/SII-k7/codex-air"
OLD_REPOSITORY_URLS = (
...
"""
    ...
    def test_readmes_use_the_canonical_repository_name(self) -> None:
        ...
        for path, text in documents.items():
            ...
            self.assertIn("codex-air", text, path.name)
    ...
    def test_readmes_publish_role_and_route_hierarchies(self) -> None:
        ...
        specs = {
            CHINESE_README: (
                "### 角色层级",
                "### 路由选择",
                ("**Controller**", "**Complex worker**", "**Efficient / Lean Primary**"),
                ("**Direct**", "**Lean Primary**", "**Parallel AIR**", "**Critical/Controller → complex**"),
            ),
            ENGLISH_README: (
                "### Role hierarchy",
                "### Route selection",
        ...
        for path, text in documents.items():
            ...
            self.assertEqual(
                ...
                list(route_order),
                f"{path.name}: route hierarchy must be Direct -> Lean -> Parallel -> complex",
            )
    ...
    def test_readmes_explain_the_two_role_runtime_and_platform_quickstarts(self) -> None:
        ...
        for path, text in documents.items():
            ...
            self.assertRegex(text, r"(?i)Controller[\s\S]{0,220}(?:controls|Full|协调|主控)")
