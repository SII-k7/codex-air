...
$testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ("codex-air-v100-windows-" + [Guid]::NewGuid().ToString("N"))
...
function Assert-V1Installed {
    ...
    $state = Get-Content -LiteralPath (Join-Path $HomePath ".codex/codex-air/install-state") -Raw
    Assert-True ($state -match '(?m)^version=7$') "AIR state version is invalid"
}
...
function Test-ModifiedTargetRefusal {
    ...
    [System.IO.Directory]::CreateDirectory($homePath) | Out-Null
    Install-At $homePath
    Add-Content -LiteralPath (Join-Path $homePath ".agents/skills/codex-air/SKILL.md") -Value "user change"
    Install-At $homePath 1
    Assert-PathExists (Join-Path $homePath ".agents/skills/codex-air/SKILL.md")
}
...
function Test-InstallRollback {
    ...
    [System.IO.Directory]::CreateDirectory($homePath) | Out-Null
    Install-At $homePath
    $before = Get-FileDigest (Join-Path $homePath ".codex/agents/air-controller.toml")
    Install-At $homePath 1 "after-replace"
    Assert-True ((Get-FileDigest (Join-Path $homePath ".codex/agents/air-controller.toml")) -eq $before) "rollback did not restore controller"
    Assert-V1Installed $homePath
}
...
function Test-V050MigrationAndRestore {
    ...
    Assert-PathAbsent (Join-Path $homePath ".agents/skills/codex-air")
    Assert-PathAbsent (Join-Path $homePath ".codex/agents/air-controller.toml")
    Assert-PathAbsent (Join-Path $homePath ".codex/agents/air-critical-controller.toml")
    Assert-PathAbsent (Join-Path $homePath ".codex/agents/air-challenger.toml")
